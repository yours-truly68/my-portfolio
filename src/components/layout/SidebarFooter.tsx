...
export const SidebarFooter = React.forwardRef<HTMLDivElement, SidebarFooterProps>(
  ({ className, ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={cn("flex flex-col gap-4 px-4 py-6 mt-auto border-t border-[var(--color-border-light)]", className)}
        {...props}
      >
        <div className="flex items-center gap-2">
          ...
          <IconButton
            ...
            onClick={() => (window.location.href = "mailto:mrazim.dev@gmail.com")}
          >
        ...
        <div className="flex items-center justify-between pt-2">
          ...
          >
            <Sun className={cn("w-3.5 h-3.5", theme === "light" && "text-[var(--color-brand-primary)]")} />
            <Moon className={cn("w-3.5 h-3.5", theme === "dark" && "text-[var(--color-brand-primary)]")} />
          </button>
        </div>
      </div>
    );
